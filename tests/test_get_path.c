...
Test(get_path1, existing_absolute){
    ...
    cr_assert_eq(get_path(&shell, &path, to_exec), SUCCESS);
...
Test(get_path2, not_existing_absolute){
    ...
    cr_assert_eq(get_path(&shell, &path, to_exec), FAILURE);
...
Test(get_path4, is_in_PATH){
    ...
    modify_env_var("PATH", &shell, path_var);
    char *path = NULL;
    char *result = strdup(get_env_var(&shell, "PATH="));
